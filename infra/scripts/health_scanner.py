...
def generate_macro_improvements():
    ...
    if missing_inits > 0:
        improvements.append(
            ...
            "This can break absolute imports across the codebase."
        )
    ...
    if len(complex_files) > 5:
        improvements.append(
            "🧩 [Architecture] High number of 'God Objects' and oversized files "
            "detected. We need to adopt a cleaner Domain-Driven Design (DDD). "
            "We should start splitting components by Domain "
            "(e.g., `core/audio` -> split into `capture`, `pipeline`, `output`)."
        )
    ...
    if ts_debt > 0:
        improvements.append(
            f"🟦 [Type Safety] {ts_debt} TypeScript shortcuts ('any' or '@ts-ignore') "
            "detected. We should strictly enforce tsconfig.json `'strict': true` to "
            "prevent future runtime UI bugs."
        )
    ...
    if long_funcs > 10:
        improvements.append(
            f"📜 [Maintainability] {long_funcs} functions exceed 80 lines. "
            "Testability is severely impaired here. Recommendation: Apply the "
            "'Extract Function' refactoring pattern heavily on the core handlers."
        )
...
def generate_report(py_count: int, ts_count: int, output_json: bool = False):
    ...
    if output_json:
        ...
        output_path = (
            ...
            / "docs"
            / "audits"
            / f"health_scan_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
