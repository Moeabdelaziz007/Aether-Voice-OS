...
async def test_event_bus_10k_eps_stress():
    ...
    # Callbacks
    async def audio_cb(event): processed_counts["Audio"] += 1
    async def control_cb(event): processed_counts["Control"] += 1
    async def telemetry_cb(event): processed_counts["Telemetry"] += 1
    ...
    for i in range(total_events):
        ...
        if i % 3 == 0:
            ...
            await bus.publish(AudioFrameEvent(timestamp=ts, source="test", latency_budget=5000, pcm_data=b"\x00"*640, sample_rate=16000, channels=1))
        elif i % 3 == 1:
            # Tier 2: Medium Priority
            await bus.publish(ControlEvent(timestamp=ts, source="test", latency_budget=5000, command="TEST_CMD", payload={}))
        else:
            # Tier 3: Low Priority (Droppable)
            await bus.publish(TelemetryEvent(timestamp=ts, source="test", latency_budget=5000, metric_name="test_metric", value=1.0))
    ...
    assert sum(processed_counts.values()) > total_events * 0.95, f"Lost too many events: {total_events - sum(processed_counts.values())} dropped."
