...
async def test_e2e_singularity():
    ...
    assert vad_res.is_hard is True, f"Failed to detect speech. RMS: {vad_res.energy_rms}"
    ...
    mock_fc1 = MagicMock()
    mock_fc1.name, mock_fc1.id, mock_fc1.args = "write_memory", "call_1", {"key": "sys_plan", "value": {"design": "Microservices"}, "tags": ["arch"]}
    mock_fc2 = MagicMock()
    mock_fc2.name, mock_fc2.id, mock_fc2.args = "write_memory", "call_2", {"key": "sys_target", "value": {"language": "Python"}, "tags": ["code"]}
    
    results = await asyncio.gather(
        router.dispatch(mock_fc1),
        router.dispatch(mock_fc2)
    )
    ...
    mock_fc_handoff.name, mock_fc_handoff.id, mock_fc_handoff.args = "delegate_to_agent", "call_3", {"target_agent_id": "CodingExpert", "task_description": "Implement the plan."}
    ...
    mock_doc.get.return_value.to_dict = MagicMock(return_value={"key": "sys_plan", "value": {"design": "Microservices"}})
    
    mock_fc_read = MagicMock()
    mock_fc_read.name, mock_fc_read.id, mock_fc_read.args = "read_memory", "call_4", {"key": "sys_plan"}
