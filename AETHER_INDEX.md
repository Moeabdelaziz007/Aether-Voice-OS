...
```mermaid
graph TD
    User((User Voice/Vision)) -->|Audio/Frame Pulse| Gateway[Aether Gateway]
    Gateway -->|Intent Extraction| Cortex[AI Cortex - Gemini]
    Cortex -->|Tool Call| Skills[Skills Hub - Autonomous Tools]
    Skills -->|Mutation| FS[(Firebase/Cloud)]
    FS -->|Global Bus| Portal[Next.js Portal - UI]
    Portal -->|Visual Feedback| User
    
    subgraph Self-Healing
        Watchdog[SRE Watchdog] -->|Monitors| Gateway
        Watchdog -->|Triggers| Diagnostic[Healer Tool]
    end
```
