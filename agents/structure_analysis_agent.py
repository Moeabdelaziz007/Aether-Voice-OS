...
class StructureAnalysisAgent:
    ...
    async def run(self) -> Dict[str, Any]:
        ...
        try:
            ...
            results["critical_issues"] = sum(
                1 for smell in self.report.code_smells if smell.severity == "critical"
            )
    ...
    async def _analyze_file(self, file_path: Path) -> None:
        ...
        try:
            ...
            # Create module info
            module_info = ModuleInfo(path=str(file_path), lines_of_code=len(lines))
            ...
            self.report.code_smells.append(
                CodeSmell(
    ...
    def _analyze_module_coupling(self) -> None:
        ...
        for module_path, info in self.report.modules.items():
            ...
            afferent = sum(
                1
                for other_info in self.report.modules.values()
    ...
    def _generate_suggestions(self) -> None:
        ...
        audio_files = [p for p in self.report.modules.keys() if "audio" in p.lower()]
