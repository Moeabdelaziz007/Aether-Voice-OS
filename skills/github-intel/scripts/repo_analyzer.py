...
def analyze_languages(path):
    ext_map = {
        ...
        '.jsx': 'JavaScript (React)', '.css': 'CSS', '.scss': 'SCSS', '.html': 'HTML',
        '.go': 'Go', '.rs': 'Rust', '.cpp': 'C++', '.h': 'C/C++', '.c': 'C',
    ...
    if total == 0: return "No detectable languages."
...
def generate_mermaid(path):
    ...
    dirs = [d for d in os.listdir(path) if os.path.isdir(os.path.join(path, d)) and d not in {'.git', 'node_modules'}]
    if not dirs: return "graph TD\n  Root --> Files"
...
def analyze_repo(url, depth=2):
    ...
    try:
        git_log = subprocess.check_output(['git', '-C', temp_dir, 'log', '-n', '5', '--pretty=format:- %ar: %s'], text=True)
        print(git_log)
    except:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Analyze GitHub Repo structure and patterns")
    parser.add_argument("url", help="GitHub Repository URL")
    parser.add_argument("--depth", type=int, default=2, help="Directory traversal depth")
