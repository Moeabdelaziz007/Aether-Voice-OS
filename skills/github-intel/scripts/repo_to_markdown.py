...
def is_text_file(file_path):
    ...
    text_extensions = {
        '.txt', '.md', '.py', '.js', '.ts', '.tsx', '.jsx', '.css', '.scss', '.html', 
        '.json', '.yaml', '.yml', '.toml', '.xml', '.go', '.rs', '.cpp', '.h', '.c', 
        '.sh', '.bash', '.sql', '.dockerfile', '.env', '.gitignore', '.cfg', '.ini'
    }
...
def get_repo_files(repo_path, max_files=100):
    ...
    for root, dirs, files in os.walk(repo_path):
        ...
        dirs[:] = [d for d in dirs if d not in {'.git', 'node_modules', '__pycache__', 'dist', 'build', 'venv', '.venv'}]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Convert GitHub Repo to AI-friendly Markdown")
