...
class StabilityTester:
    ...
    async def run_resource_monitor(self):
        ...
        while time.time() < end_time:
            ...
            if len(self.results["telemetry"]) % 10 == 0:
                print(
                    f"  [TELEMETRY] CPU: {cpu_percent}% | RAM: {stats['rss_mb']:.2f}MB",
                    flush=True,
                )
    ...
    async def run_bus_stress(self, target_ops: int = 5000):
        ...
        print(
            f"⚡ Starting Bus Stress Test ({target_ops} msgs/sec target)...", flush=True
        )
        ...
        print(
            f"✅ Bus Stress Complete: {count / total_duration:.2f} msgs/sec", flush=True
        )
