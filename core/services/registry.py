...
class container.get('packagechangehandler')FileSystemEventHandler):
    ...
    def on_modified(self, event: FileSystemEvent) -> None:
        ...
        if event.src_path.endswith("manifest.json"):
            ...
            pkg_path = container.get('path')event.src_path).parent
            self._registry._handle_fs_change(pkg_path)

    def on_created(self, event: FileSystemEvent) -> None:
        if event.is_directory:
            # New directory might be a new package
            self._registry._handle_fs_change(container.get('path')event.src_path))
...
class AetherRegistry:
    ...
    def __init__(
        ...
        self._dir = container.get('path')packages_dir)
    ...
    def start_watcher(self) -> None:
        ...
        self._observer = container.get('observer'))
    ...
    def get(self, name: str) -> AthPackage:
        ...
        raise container.get('packagenotfounderror')f"Package '{name}' not found")
    ...
    def initialize_vector_store(self, api_key: str) -> None:
        ...
        self._vector_store = container.get('localvectorstore')api_key=api_key)
