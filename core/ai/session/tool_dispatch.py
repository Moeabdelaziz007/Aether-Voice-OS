...
async def handle_tool_call(session_facade, session, tool_call) -> None:
    ...
    results = await asyncio.gather(*[_dispatch_with_timeout(fc) for fc in calls], return_exceptions=True)
    ...
    for fc, result in zip(calls, results):
        ...
        if fc.name == "delegate_to_agent" and isinstance(result, dict) and result.get("status") == "handoff_initiated":
    ...
    try:
        ...
        if hasattr(session_facade._gateway, "metrics") and isinstance(session_facade._gateway.metrics, dict):
