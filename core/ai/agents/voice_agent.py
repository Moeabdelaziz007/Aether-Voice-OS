...
class VoiceAgent:
    ...
    def build_dna_prompt(self, base_instructions: str) -> str:
        ...
        if self.dna.empathy > 0.8:
            modifiers.append("Use a warm, highly empathetic tone. Validate user emotions.")
        elif self.dna.empathy < 0.2:
            modifiers.append("Maintain a strictly professional, robotic, and efficient tone.")
            
        # Proactivity
        if self.dna.proactivity > 0.7:
            modifiers.append("Be proactive. Anticipate needs and suggest tool usage early.")
