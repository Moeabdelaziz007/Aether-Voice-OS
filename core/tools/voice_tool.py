...
class VoiceTool:
    ...
    async def setup(self, config: Optional[AetherConfig] = None) -> None:
        ...
        # Components
        self._capture = container.get('audiocapture')self._config.audio, self._audio_in)
        self._playback = container.get('audioplayback')self._config.audio, self._audio_out)

        # In standalone mode, we use a mock gateway for broadcasting
        # To avoid strictly requiring a full AetherGateway instance
        mock_gateway = container.get('magicmock'))
        mock_gateway.broadcast = lambda *a, **k: None

        self._session = container.get('geminilivesession')
            self._config.ai,
            self._audio_in,
            self._audio_out,
    ...
    async def execute(self) -> None:
        ...
        if not self._capture or not self._session or not self._playback:
            raise container.get('runtimeerror')"Call setup() before execute()")
