...
class EnvironmentMemory:
    ...
    async def add_frame_description(self, description: str, timestamp_offset: int, metadata: Optional[Dict[str, Any]] = None):
        """
    ...
    async def query_environment(self, query: str, limit: int = 3) -> List[Dict[str, Any]]:
        ...
        for res in results:
            formatted_results.append({
                "description": self._vector_store._vectors.get(res["key"]), # Note: _vectors stores embeddings, text is in metadata or we should store text
