...
class CloudFunctions:
    ...
    def on_session_start(session_id: str, user_id: str):
        ...
        """
        logger.info(
            f"CloudFn [onSessionStart]: Initializing metrics for session {session_id} (User: {user_id})"
        )
        # In a real environment:
        # db.collection('sessions').document(session_id).set({'user_id': user_id, 'start_time': firestore.SERVER_TIMESTAMP})

    @staticmethod
    def aggregate_emotions(session_id: str):
        """
        Triggered periodically or on session end.
        Aggregates raw EmotionalEvents into summary vectors (e.g., peak frustration, average valence).
        """
        logger.info(
            f"CloudFn [aggregateEmotions]: Rolling up affective data for session {session_id}"
        )
        # In a real environment:
